...
export interface PublishingRepository {
  ...
  saveAttempt(accountId:string, attempt:PublishAttempt):Promise<PublishAttempt>;
  getLatestAttempt(accountId:string, brandId:string, commandId:string):Promise<PublishAttempt|null>;
  savePublishedPost(accountId:string, post:PublishedPost):Promise<PublishedPost>;
}

...
import { applyPublishAttempt, beginPublishAttempt, cancelPublishCommand, connectChannelAccount, createPublishCommand, createPublishedPost, reconcilePublishAttempt, retryPublishCommand, type ChannelAccount, type PublishAttempt, type PublishCapability, type PublishChannel, type PublishCommand, type PublishContentType, type PublishMediaItem, type PublishOptions, type PublishedPost } from "./publishing";
...
export class PublishingService {
  ...
  async schedule(accountId:string,brandId:string,campaignId:string,assetId:string,input:{channelAccountId:string;contentType:PublishContentType;mediaItems?:PublishMediaItem[];options?:PublishOptions;scheduledFor:string}){const detail=await this.campaigns.getCampaign(accountId,brandId,campaignId);if(!detail)throw new ResourceNotFoundError("Campaign not found");const entry=detail.assets.find(x=>x.asset.id===assetId);if(!entry)throw new ResourceNotFoundError("Content Asset not found");const version=entry.versions.at(-1);if(!version)throw new ResourceNotFoundError("Content Version not found");const approval=await this.reviews.getApproval(accountId,brandId,assetId);if(!approval)throw new ResourceNotFoundError("Content Approval not found");const channel=await this.publishing.getChannelAccount(accountId,brandId,input.channelAccountId);if(!channel)throw new ResourceNotFoundError("Channel Account not found");return this.publishing.saveCommand(accountId,createPublishCommand({id:randomUUID(),approval,currentVersionId:version.id,channelAccount:channel,contentType:input.contentType,mediaItems:input.mediaItems,options:input.options,scheduledFor:input.scheduledFor,createdAt:this.now().toISOString()}))}

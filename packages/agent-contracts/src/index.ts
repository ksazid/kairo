...
export type AgentRole = "hunter" | "researcher" | "strategist";
...
export function prepareAgentInvocation(input: AgentInvocationInput): AgentInvocationRequest {
  if (!["hunter", "researcher", "strategist"].includes(input.role)) throw new AgentContractError("agent role is not enabled");

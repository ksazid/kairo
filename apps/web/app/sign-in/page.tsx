...
export default async function SignInPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  async function submit(formData: FormData) {
    ...
    if (!email || !password || (mode === "signup" && !name)) redirect(`/sign-in?mode=${mode === "signup" ? "signup" : "signin"}&error=Missing%20required%20fields`);

    const result = mode === "signup"
      ? await auth.signUp.email({ email, password, name })
      : await auth.signIn.email({ email, password });
    if (result.error) redirect(`/sign-in?mode=${mode === "signup" ? "signup" : "signin"}&error=${encodeURIComponent(result.error.message)}`);

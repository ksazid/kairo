...
export default async function Home({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (session.workspaces.length === 0) {
    return <Onboarding displayName={session.account.displayName ?? session.account.email ?? "there"} />;
  }
      ...
      <aside className="sidebar" aria-label="Primary navigation">
        ...
        <nav className="nav-list">
          {primaryNav.map((item, index) => (
            index === 0 ? (
              <Link key={item} className="nav-item active" href="/" aria-current="page">{item}</Link>
            ) : (
              <span key={item} className="nav-item disabled" aria-disabled="true">{item}<small>Later</small></span>
            )
          ))}
        </nav>
      ...
      <main className="workspace-main">
        <header className="topbar">
          <div>
            ...
            <p className="lede">Kairo is establishing the trusted Brand boundary before intelligence begins.</p>
          </div>
          ...
          <article className="status-panel">
            <div className="panel-heading">
              <div>
                ...
                <h2>Account, Workspace and Brand</h2>
              </div>
            ...
            <div className="check-list">
              <span>Account boundary established</span>
              <span>Workspace membership verified</span>
              <span>Brand scope explicit</span>
            </div>
          </article>
          ...
          <aside className="scope-panel">
            ...
            <div className="brand-list" aria-label="Available Brands">
              {brands.map((item) => (
                ...
                >
                  <span>{item.name}</span>
                  <small>{item.id === brand?.id ? "Current" : "Switch"}</small>
                </Link>
              ))}
            </div>
          </aside>
        ...
        <section className="next-panel">
          ...
          <p>Kairo will not invent opportunities just to make this screen look busy. Brand Brain, knowledge, discovery and research arrive in their governed slices.</p>
        </section>
      </main>

      <nav className="mobile-nav" aria-label="Mobile navigation">
        {mobileNav.map((item, index) => index === 0 ? (
          <Link key={item} href="/" className="mobile-nav-item active" aria-current="page">{item}</Link>
        ) : (
    ...
    <main className="onboarding-page">
      <section className="onboarding-card">
        ...
        <form action={createWorkspaceAction} className="onboarding-form">
          ...
          <label>
            Brand name
            <input name="brandName" required maxLength={120} placeholder="Acme" />
          </label>
          <label>
            Website <span>optional</span>
            <input name="publicSourceUrl" type="url" placeholder="https://example.com" inputMode="url" />
          </label>
          <label>
            Public profile <span>optional</span>
            <input name="publicProfileUrl" type="url" placeholder="https://linkedin.com/company/..." inputMode="url" />
          </label>

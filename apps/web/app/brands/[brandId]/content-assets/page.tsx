...
export default async function ContentAssetsPage({params,searchParams}:{params:Params;searchParams:SearchParams}){
  ...
  return <KairoProductShell brandId={brand.id} workspaceId={brand.workspaceId} active={null} mobileActive="More">
    <main className={`workspace-main ${styles.main}`} id="kairo-main-content" tabIndex={-1}>
      ...
      {query.created?<p className={styles.notice} role="status">Library created. Connect and index provider content in the next approved connector slice.</p>:null}
      ...
      <div className={styles.layout}>
        <aside className={styles.panel} aria-labelledby="asset-libraries-title">
          ...
          <div className={styles.libraryList}>
            {libraries.length?libraries.map((library)=><div className={styles.libraryRow} key={library.id}><strong>{library.name}</strong><div className={styles.libraryMeta}><span>{providerName(library.provider)}</span><span className={styles.status} data-status={library.status}>{statusName(library.status)}</span></div></div>):<div className={styles.libraryRow}><strong>No libraries yet</strong><div className={styles.libraryMeta}>Create one without connecting a provider.</div></div>}
          </div>
          <details className={styles.create}>
            ...
            <form className={styles.form} action={create}>
              ...
              <p className={styles.providerNote}>This creates the library slot only. Google authorization and Drive indexing remain disabled until VS-59B receives separate approval.</p>
        ...
        <section className={styles.assets} aria-labelledby="asset-browser-title">
          <div className={styles.assetsHeader}><div><h2 id="asset-browser-title">Asset browser</h2><p>{assets.length?`${assets.length} indexed ${assets.length===1?"asset":"assets"}`:"No indexed assets in this view"}</p></div><Link className="secondary-button" href={`/brands/${encodeURIComponent(brand.id)}/campaigns`}>Back to Content Studio</Link></div>

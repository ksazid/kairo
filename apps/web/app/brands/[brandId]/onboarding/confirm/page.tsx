  ...
  return (
    <main className={styles.page}>
      <section className={`${styles.surface} ${styles.onboardingSurface}`} aria-labelledby="brand-confirm-title">
        ...
        {readiness?.status === "ready" ? <p className={confirmStyles.readiness} role="status">✓ Brand DNA ready · {readiness.brandIntelligenceScore}% intelligence</p> : readiness?.nextAction ? <div className={confirmStyles.readinessNotice} role="status"><strong>Kairo needs one more thing before it can find relevant opportunities.</strong><p>Brand Intelligence: {readiness.brandIntelligenceScore}% · {readiness.nextAction.prompt}</p><form action={enrichOnboardingBrandAction.bind(null, brand.id)} className={confirmStyles.enrichmentForm}>{readiness.nextAction.type === "add-source" ? <><input type="hidden" name="kind" value="source" /><input name="publicReferenceUrl" type="url" required placeholder="https://yourbrand.com" aria-label="Public Brand link" /><button className="secondary-button" type="submit">Add source</button></> : readiness.nextAction.type === "confirm-none" ? <><input type="hidden" name="kind" value="none" /><button className="secondary-button" type="submit">Confirm none</button></> : <><input type="hidden" name="kind" value="field" /><input type="hidden" name="fieldKey" value={readiness.nextAction.fieldKey} /><input name="value" required placeholder="Type your answer" aria-label={readiness.nextAction.prompt} /><button className="secondary-button" type="submit">Save answer</button></>}</form></div> : null}

...
export async function GET(request:Request,{params}:{params:Promise<{brandId:string}>}){
  ...
  try{const{authorizationUrl}=await beginInstagramConnection(brandId);return NextResponse.redirect(authorizationUrl)}catch(error){const message=error instanceof Error?error.message:"Unable to start Instagram connection";return NextResponse.redirect(new URL(`/brands/${encodeURIComponent(brandId)}/performance?error=${encodeURIComponent(message)}`,request.url))}
}

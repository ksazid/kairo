...
export interface ContentApprovalView { id:string;versionId:string;version:number;reviewId:string;approverAccountId:string;destination:{channel:"linkedin"|"instagram"|"manual";accountRef:string};approvedAt:string }
export interface ContentReviewStatusView { review:ContentReviewView|null;approval:ContentApprovalView|null }
export interface ChannelAccountView{id:string;workspaceId:string;brandId:string;channel:"linkedin"|"instagram"|"manual";accountRef:string;displayName:string;capabilities:Array<"publish-text"|"publish-image"|"publish-video"|"publish-carousel">;status:"connected"|"reconnect-required"|"disabled";connectedAt:string}
export interface PublishCommandView{id:string;workspaceId:string;brandId:string;campaignId:string;assetId:string;versionId:string;version:number;approvalId:string;channelAccountId:string;channel:"linkedin"|"instagram"|"manual";accountRef:string;contentType:"text"|"image"|"video"|"carousel";scheduledFor:string;status:"scheduled"|"dispatching"|"published"|"failed"|"unknown"|"manual-required"|"cancelled";attemptCount:number;createdAt:string;lastAttemptAt?:string}
...
export async function approveContentVersionRequest(brandId:string,campaignId:string,assetId:string,input:{expectedVersion:number;destination:{channel:"linkedin"|"instagram"|"manual";accountRef:string}}):Promise<ContentApprovalView>{return bodyOrError(await authorizedFetch(`/api/v1/brands/${encodeURIComponent(brandId)}/campaigns/${encodeURIComponent(campaignId)}/assets/${encodeURIComponent(assetId)}/approve`,{method:"POST",body:JSON.stringify(input)}),"Unable to approve Content Version")}

...
export function getHomeMedia(brandId: string) {
  return call<HomeMediaAssetView[]>(`/api/v1/brands/${encodeURIComponent(brandId)}/home-media`);
}
export function beginHomeMediaUpload(brandId: string, input: { name: string; mimeType: string; sizeBytes: number }) {
  return call<BeginHomeMediaUploadView>(`/api/v1/brands/${encodeURIComponent(brandId)}/home-media/uploads`, {
    method: "POST",
    body: JSON.stringify(input),
  });
}
export function completeHomeMediaUpload(brandId: string, uploadId: string) {
  return call<HomeMediaAssetView>(
    `/api/v1/brands/${encodeURIComponent(brandId)}/home-media/uploads/${encodeURIComponent(uploadId)}/complete`,
    { method: "POST", body: "{}" },
  );
}

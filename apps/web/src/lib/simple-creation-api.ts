...
async function call<T>(path: string, init?: RequestInit) {
  ...
  if (!token) throw new SimpleCreationApiError("Authentication is required");
...
export function startSimpleCreation(
  brandId: string,
  input: {
    goal: string;
    input?: string;
  ...
  return call<SimpleCreationView>(
    `/api/v1/brands/${encodeURIComponent(brandId)}/simple-creations`,
    { method: "POST", body: JSON.stringify(input) },
  );
}
export function getSimpleCreation(brandId: string, id: string) {
  return call<SimpleCreationView>(
    `/api/v1/brands/${encodeURIComponent(brandId)}/simple-creations/${encodeURIComponent(id)}`,
  );
}

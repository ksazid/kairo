...
describe("approved product navigation", () => {
  it("keeps stable internal destination keys on desktop and mobile", () => {
    expect(DESKTOP_PRODUCT_DESTINATIONS).toEqual(["Home","Content","Discover","Calendar","Results","Brand"]);
  ...
  it("renders Results as the approved user-facing Insights label", () => {
    ...
    expect(navigation.desktop.map((item) => item.displayLabel)).toEqual(["Home","Content","Discover","Calendar","Insights","Brand"]);

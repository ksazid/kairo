...
export async function saveContentVersionProductionAssets(brandId:string,campaignId:string,assetId:string,input:{expectedVersion:number;libraryAssetIds:string[]}):Promise<void>{await bodyOrError(await authorizedFetch(`/api/v1/brands/${encodeURIComponent(brandId)}/campaigns/${encodeURIComponent(campaignId)}/assets/${encodeURIComponent(assetId)}/library-assets`,{method:"POST",body:JSON.stringify(input)}),"Unable to save production assets")}

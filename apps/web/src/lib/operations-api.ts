...
export async function getOperations(brandId:string):Promise<OperationsSummaryView>{return body(await kairoServerFetch(`/api/v1/internal/brands/${encodeURIComponent(brandId)}/operations`),"Unable to load Pilot Operations")}
export async function requestOperationsRetry(brandId:string,failureId:string,idempotencyKey:string):Promise<RetryRequestView>{return body(await kairoServerFetch(`/api/v1/internal/brands/${encodeURIComponent(brandId)}/operations/failures/${encodeURIComponent(failureId)}/retry`,{method:"POST",body:JSON.stringify({idempotencyKey,requestedAt:new Date().toISOString()})}),"Unable to retry operation")}
export async function disableOperationsAutomation(brandId:string,automationKey:string,expectedVersion:number,reason:string):Promise<AutomationControlView>{return body(await kairoServerFetch(`/api/v1/internal/brands/${encodeURIComponent(brandId)}/operations/automations/${encodeURIComponent(automationKey)}/disable`,{method:"POST",body:JSON.stringify({expectedVersion,reason,at:new Date().toISOString()})}),"Unable to disable automation")}

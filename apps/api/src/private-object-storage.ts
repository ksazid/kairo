...
export class S3PrivateCreativeObjectStore implements CreativeObjectStorePort {
  ...
  async putPrivateObject(input: {
    ...
    if (!safeHash(input.contentHash, hash)) {
      throw new Error("Private object content hash does not match bytes");
    }
...
/**
 * Creates bounded browser-upload URLs without exposing storage credentials.
 * The caller must derive/authorize the object key; the browser never chooses it.
 */
...
/** Opaque Brand-scoped key for user-managed media. */
export function privateMediaObjectKey(workspaceId: string, brandId: string, assetId: string) {
  const id = segment(assetId, "assetId", 200);
  return `media/${brandScopeHash(workspaceId, brandId)}/${id}/original`;
}
...
class S3SigV4Client {
  ...
  presignGet(key: string, ttlInput: number): string {
    return this.presign({ method: "GET", key, expiresInSeconds: ttlInput, signedHeaders: { host: null } });
  }
...
function scopedGeneratedKey(workspaceId: string, brandId: string, key: string) {
  ...
  if (!normalized.startsWith(`generated/${expected}/`)) {
    throw new Error("Private creative object key is outside Brand scope");
  }
  return normalized;
}

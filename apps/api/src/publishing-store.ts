...
export class MemoryPublishingRepository implements PublishingRepository {
  ...
  async cancelCommand(a:string,b:string,id:string){await this.scope(a,b);const c=this.commands.get(id);if(!c||c.brandId!==b)throw new ResourceNotFoundError("Publish Command not found");const next={...c,status:"cancelled"as const};this.commands.set(id,next);return structuredClone(next)}
  async recordDispatch(a:string,command:PublishCommand,attempt:PublishAttempt){await this.scope(a,command.brandId);this.commands.set(command.id,structuredClone(command));this.attempts.set(attempt.id,structuredClone(attempt));return structuredClone(attempt)}
  ...
  async recordOutcome(a:string,command:PublishCommand,attempt:PublishAttempt,p?:PublishedPost){await this.scope(a,command.brandId);this.commands.set(command.id,structuredClone(command));this.attempts.set(attempt.id,structuredClone(attempt));if(p)this.posts.set(p.id,structuredClone(p));return structuredClone(command)}

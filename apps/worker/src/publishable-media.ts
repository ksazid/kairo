...
function parseManifest(bytes:Uint8Array,pkg:StoredCreativePackage):ReelManifest{
  ...
  const target=finite(m.targetDurationSeconds,"targetDurationSeconds");if(target<=0||!scenes.length||Math.abs(scenes[scenes.length-1]!.endSecond-target)>0.001)throw new Error("Reel target duration does not match scenes");

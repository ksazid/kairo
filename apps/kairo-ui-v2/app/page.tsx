  ...
  return <KairoShell active="Home" authenticated={data.authenticated} brandId={data.brandId} brandName={data.brandName}>
        ...
        <section className="recommendation">
          ...
          <div className="recommend-grid">
            {featuredMockup ? <div className="hero-image concept-home-preview"><ConceptMockupPreview mockup={featuredMockup} mode="compact"/></div> : <div className="hero-image home-opportunity-visual"><FormatIcon aria-hidden="true"/><span>Grounded opportunity</span><strong>{compactOpportunityTitle(featured.title)}</strong></div>}
            <div className="recommend-copy"><h3>{selectedFormat === "campaign" ? `Build a campaign around: ${compactOpportunityTitle(featured.title)}` : compactOpportunityTitle(featured.title)}</h3><h4><ShieldCheck aria-hidden="true"/>Why this fits your Brand</h4><p>{compactOpportunityText(featured.rationale, "This opportunity is available because it passed the configured Brand-fit checks.", 24)}</p><h4 className="trend-copy"><TrendingUp aria-hidden="true"/>{previewMode ? "What happens next" : "Why it is trending"}</h4><p>{compactOpportunityText(featured.whyNow, "This opportunity has persisted evidence from the latest Discovery run.", 24)}</p><div className="actions"><CreateButton brandId={data.brandId} opportunityId={data.brandId && !previewMode ? featured.id : undefined} title={featured.title} direction={featured.developmentDirection ?? featured.rationale} format={selectedFormat}/>{previewMode ? <Link href={discoverHref}>Open Discovery</Link> : <><Link href={`/?${nextQuery.toString()}`}>See another</Link><Link href={featuredHref}>View evidence</Link></>}</div></div>

...
async function capture({name,width,height,mobile}){
  ...
  try{
    ...
    const cookie=await send("Network.setCookie",{name:"kairo_access_token",value:"vs92-ui-token",url:"http://127.0.0.1:3000/",path:"/",httpOnly:true,sameSite:"Lax"});
    if(cookie.success===false)throw new Error("Unable to set session cookie");
    ...
    const expression=`(() => {
      ...
      const getRect = (selector) => { const el = document.querySelector(selector); return el ? rect(el) : null; };
      return {
        scrollWidth: document.documentElement.scrollWidth,
        clientWidth: document.documentElement.clientWidth,
        scrollHeight: document.documentElement.scrollHeight,
        clientHeight: document.documentElement.clientHeight,
        homeText: document.querySelector('#kairo-main-content')?.innerText?.replace(/\\s+/g,' ').trim().slice(0,1600) || '',
        mobileHeader: getRect('.k-shell-mobile-header'),
        attention: getRect('[aria-label="Needs attention"] article'),
        idea: getRect('#my-idea'),
        forYou: getRect('[aria-labelledby="home-for-you-title"]'),
        working: getRect('[aria-labelledby="home-working-title"]'),
        bottomNav: getRect('.k-shell-mobile-nav'),
        toolCells: [...document.querySelectorAll('[aria-label="Idea sources"] button')].map(rect),
        metrics: [...document.querySelectorAll('[aria-label="Latest available Brand performance"] article')].map(rect)
      };
    })()`;
    const domResult=await send("Runtime.evaluate",{expression,returnByValue:true});
    const dom=domResult.result?.value;
    if(!dom)throw new Error(`Unable to read DOM geometry for ${name}: ${JSON.stringify(domResult)}`);
    const shot=await send("Page.captureScreenshot",{format:"png",fromSurface:true,captureBeyondViewport:false});
    await writeFile(`${outputDir}/${name}.png`,Buffer.from(shot.data,"base64"));
    return {name,width,height,mobile,dom};

# VS-90 architecture decision — Avatar / Presenter boundary

Decision status: **approved**. Tracking issue: #188. Decision record: `DEC-012`.
...
## Approved — Kairo-owned Presenter state + replaceable AvatarProvider

- PostgreSQL stores authoritative Brand-scoped Presenter profile/version/readiness state.
- Private object storage stores visual/audio references and generated presenter media when provider execution is later enabled.
...
## Rejected — Provider-owned Presenter profile

Kairo stores only a provider presenter/avatar ID and reads most state from the provider.

Rejected because it makes provider availability authoritative over Brand state, weakens portability/versioning and complicates exact approved-asset lineage.

## Rejected — Frontend-only Presenter preferences until provider work arrives

Store or simulate Presenter preferences in the web layer without a complete server/provider contract.

Rejected because it creates fake product state, cannot enforce tenancy or immutable approval lineage, and conflicts with PostgreSQL-as-system-of-record.

## Consequences

1. Presenter contracts/domain records and migration belong to the VS-90 foundation.
2. A replaceable `AvatarProvider` capability port is required; the default implementation fails closed when no provider is configured.
3. A future production-capable adapter may target a configured custom/self-hosted endpoint; no provider is hard-coded into product policy.
4. AI & Media Providers Settings remains a later slice; VS-90 consumes server-side provider capability only.
